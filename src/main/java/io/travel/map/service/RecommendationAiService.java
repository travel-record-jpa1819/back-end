...
public class RecommendationAiService {
    ...
    public List<TravelRecommendationDto> recommendCitiesBasedOnUser(String userId) {
        ...
        try {
            return objectMapper.readValue(
                    rawResponse,
                    new TypeReference<List<TravelRecommendationDto>>() {}  // 중요
            );
        } catch (Exception e) {
    ...
    private String buildJsonPrompt(User user) {
        ...
        user.getVisitedCities().forEach(visited -> {
            sb.append("- ").append(visited.getCityID()).append(": ");
            sb.append(visited.getDescription() != null ? visited.getDescription() : "No description");
        ...
        sb.append("""
                ...
                Recommend exactly 5 cities for the user to visit, in JSON array format only.
                
                Return an array of objects (no markdown, no backticks) with a short reason why did you recommend that city.
